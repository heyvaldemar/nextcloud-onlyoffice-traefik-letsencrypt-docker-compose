#!/bin/bash
# End-to-end tests for the nextcloud-onlyoffice-traefik-letsencrypt-docker-compose backup + restore flow.
#
...
COMPOSE_PROJECT_NAME="${COMPOSE_PROJECT_NAME:-nextcloud-onlyoffice}"
DOCKER_COMPOSE_FILE="${DOCKER_COMPOSE_FILE:-nextcloud-onlyoffice-traefik-letsencrypt-docker-compose.yml}"
...
DB_CONTAINER="$(docker compose -f "$DOCKER_COMPOSE_FILE" -p "$COMPOSE_PROJECT_NAME" ps -aq postgres-nextcloud | head -n 1)"
