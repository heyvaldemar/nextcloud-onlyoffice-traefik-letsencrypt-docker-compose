...
DB_HOST_FOR_ALIGN="postgres-nextcloud"
...
if ! docker exec "$BKP" sh -c "(set -o pipefail) 2>/dev/null && set -o pipefail; set -eu
    dropdb --force -h postgres-nextcloud -U '$DB_USER' --if-exists '$DB_NAME'
    createdb -h postgres-nextcloud -U '$DB_USER' '$DB_NAME'
    gunzip -c '$DIR/$SELECTED' | psql -q -v ON_ERROR_STOP=1 -h postgres-nextcloud -U '$DB_USER' -d '$DB_NAME' >/dev/null"; then
